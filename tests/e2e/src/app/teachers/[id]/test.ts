 ...
 *
 * ## Data Represented
 * - Teacher profile: name (cn/en), department, title
 * - Sections taught (table with links to /sections/[jwId])
 * - User-editable description panel
 * - Comments aggregated from teacher source
 * - Seed teacher: DEV-T-001 "王测试" (id resolved via search navigation)
 *
 * ## UI/UX Elements
 * - Breadcrumbs: Home > Teachers > {teacher.name}
 * - h1: teacher name (primary + secondary)
 * - Tabs: "sections" (default), "comments" (with count)
 * - Sidebar: Description panel (editable), Basic Info card
 * - CommentsSection with post/edit/delete
 *
 * ## Edge Cases
 * - Invalid id → notFound() → 404 page
 * - Teacher IDs are dynamic; all tests navigate via search first
 * - Description edit requires login (signInAsDebugUser)
 * - Comment CRUD: post → edit → delete (with API waits)
 */
...
test.describe("/teachers/[id]", () => {
  ...
  test("section links visible on teacher detail", async ({
    page,
  }, testInfo) => {
