...
test("/api/mcp 缺少 mcp:tools scope 时返回 insufficient_scope", async ({
  ...
  expect(response.headers()["www-authenticate"]).toContain('scope="mcp:tools"');
...
test("OAuth PKCE token can connect to /api/mcp and call all seeded tools", async ({
  ...
  try {
    ...
    expect(typeof overviewPayload.overview?.todaySchedulesCount).toBe("number");
    expect(typeof overviewPayload.overview?.upcomingExamsCount).toBe("number");
