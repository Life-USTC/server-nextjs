...
* **受保护 API 的当前鉴权形态**：当前普通用户侧受保护 `/api/*` endpoint 同时接受站内 session 与 OAuth bearer token；站内 session 鉴权以当前请求 headers 中的 cookie 为准，OAuth 调用的权限判断、字段可见性和错误语义与 Web / MCP 保持一致。管理员接口仍不作为 OAuth client 的默认能力对外开放。
...
* 静态导入约束：
  * 当静态 SQLite 快照未提供官方 `jwId` 时，`scheduleGroup.jwId` 使用稳定的站内合成值。
  * 对当前生产静态导入范围（近期学期）保持既有合成值；仅在全量历史导入出现冲突时回退到另一稳定值以保证唯一性。
...
* 考试信息：
  ...
  * `exam.examRooms[]`
* 静态导入约束：
  * 当静态 SQLite 快照未提供官方 `jwId` 时，`exam.jwId` 使用稳定的站内合成值。
...
* 权限设计：
  ...
  * 在正式同源部署上，OAuth 登录回调应直接完成状态校验与会话创建，不应因为代理态包装或容器内网 host 差异而丢失 state。
  * 新用户首次登录时，如果未设置姓名或用户名，需要先完成欢迎流程。
  * 带 OAuth `code`/`error` 与 `state` 的授权回调续接请求应允许完成协议跳转；欢迎页拦截不应破坏授权结果回跳。
...
* 相关接口：
  ...
  * 设置中心：
    * Web：
      ...
      * `/settings?tab=danger`
      * `profile`：编辑姓名、用户名、头像。
      * `accounts`：查看和管理已连接的 OAuth 账户；断开账户属于破坏性操作，必须先显示确认弹窗；尝试断开最后一个可登录账户时必须报错。
      * `content`：说明上传与评论是依附对象的内容，不提供独立“我的评论 / 我的上传”设置页；当前提供到班级浏览与评论指南的下一步入口。
      * `danger`：删除账户；删除属于高风险操作，需要输入固定确认词 `DELETE`。
      * 设置页当前只以 `?tab=` query 形式作为稳定入口，不再提供 `/settings/profile`、`/settings/accounts`、`/settings/content`、`/settings/danger`、`/settings/comments`、`/settings/uploads` 等别名入口。
    * API：
      * 当前没有单独的设置中心聚合接口。
    * MCP：
      * 不提供账户连接管理或账户删除能力。
...
* 相关接口：
  ...
  * 当前学期：
    * 模型引用：
      * [SemesterSummary](#model-semestersummary)
    * 选择规则：
      * 当前学期由日期范围判定；当多个学期范围同时覆盖当前日期时，选择 `startDate` 最新的学期，以避免旧范围或导入残留遮蔽更具体的新学期。
    * Web：
...
* 相关逻辑：
  ...
  * 日历地点与建筑图片增强只读取 `https://static.life-ustc.tiankaima.dev` 上发布的静态 JSON；加载失败或数据损坏时应记录诊断信息且仍返回有效 iCal。
* 相关接口：
  * 班级详情日历弹窗：
    * 界面引用：
      * [UI: Calendar Export Dialog](#ui-calendar-export-dialog)
...
* 相关逻辑：
  ...
  * E2E 默认使用本地 S3 兼容测试端点；只有显式提供可用 S3 环境时才直连外部存储。
  * 下载不能因拿到直链而绕过授权。
...
* 相关逻辑：
  ...
  * 仪表盘校车页在客户端根据用户本地时间默认判定工作日/周末，并允许手动切换。
  * 仪表盘客户端在计算“多久后发车”、班次是否已发车以及路线排序时，统一按上海时间解释时刻表里的 `HH:mm` 时间，不随浏览器所在时区改变。
  * 仪表盘校车页只提供出发站、到达站、方向反转和“是否显示已发车班次”这几项核心筛选；出发站和到达站采用可直接点击的纵向站点列，避免二级下拉选择，并保持清晰的出发/到达层级。
  * 仪表盘视觉结构应优先服务“快速扫读下一班与路线差异”：已选站点、方向、下一班时间和路线分组必须比装饰性卡片更醒目。
  * 当前按“所选出发站的下一班可乘时间”对所有可用路线排序；若某站在原始时刻表中没有精确时间，则使用同一班次相邻已知站点时间的平均值估算。
  * 仪表盘主视图使用一个合并的大表展示所有适用路线，并按路线分组为表格 section；每个 section 下展示该路线的可见班次。
  * `/api/bus` 与 `query_bus_timetable` 返回所选时刻表版本内的原始路线/站点/班次数据；匿名调用方的 `preferences` 为 `null`，已认证调用方返回自己的已保存偏好；服务端不预先做推荐、过滤或排序。
  * `list_bus_routes` 只列出当前生效时刻表版本内实际存在的路线，不暴露当前版本外的历史路线 ID。
  * 仪表盘优先回答“下一班怎么走”，地图页承担空间理解任务。
* 相关接口：
  * 仪表盘校车页：
    * Web：
      * `/?tab=bus`
      * 仪表盘路线与班次视图支持客户端工作日/周末切换、纵向出发站/到达站选择、反转方向、切换是否显示已发车班次，并以按路线分组的大表展示所有适用路线。
    * API：
      * `/api/bus`
      * `/api/bus/preferences`
    * MCP：
...
* 相关逻辑：
  ...
  * 固定/取消固定写入失败时，接口返回明确错误；不会把失败伪装成成功。
* 相关接口：
  * 链接浏览与搜索：
    * Web：
      * `/?tab=links`
...
* 相关接口：
  ...
  * 校车管理页：
    * Web：
      * `/admin/bus`
      * 后台手动导入静态校车数据时，应直接从 `https://static.life-ustc.tiankaima.dev` 读取已发布静态数据，不能依赖本地 clone、工作目录外路径或 `git` 命令。
      * 删除未激活时刻表版本必须使用站内 destructive confirm dialog，不能依赖浏览器原生确认框。
    * API：
...
* 相关逻辑：
  ...
  * 对于带 path 的 OAuth issuer（当前为 `/api/auth`），授权服务器元数据的规范主入口应为 `/.well-known/oauth-authorization-server/api/auth`；根级 `/.well-known/oauth-authorization-server` 仅作为兼容别名重定向到规范地址，避免在根级返回与 issuer 不一致的元数据。
  * 对于带 path 的 OpenID issuer（当前为 `/api/auth`），OIDC Discovery 的规范主入口应为 `/api/auth/.well-known/openid-configuration`，并同时提供 `/.well-known/openid-configuration/api/auth` 作为 RFC 8414 兼容路径；根级 `/.well-known/openid-configuration` 仅作为兼容别名重定向到规范地址。
  * 对于带 path 的受保护资源（当前 MCP resource 为 `/api/mcp`），RFC 9728 的规范主入口应为 `/.well-known/oauth-protected-resource/api/mcp`；根级 `/.well-known/oauth-protected-resource` 仅作为兼容别名重定向到规范地址，避免返回与 `resource` 字段不一致的元数据。
  * 兼容别名应使用重定向而不是重复返回一份“看起来能用但与 issuer/resource 校验不一致”的 JSON，以便客户端最终在规范地址上完成元数据校验。
  * Discovery 元数据应支持跨域读取：至少对 OpenID discovery 返回 `Access-Control-Allow-Origin: *`，并保持授权服务器元数据与受保护资源元数据的 CORS 行为一致，降低浏览器型客户端与调试工具的兼容性风险。
* 相关接口：
  * 授权页：
    * Web：
      * `/oauth/authorize`
...
* 相关逻辑：
  ...
  * 当前普通用户侧业务 REST endpoint 同时支持当前请求 cookie 中的站内 session 与 OAuth bearer token，bearer-token 访问不再局限于 MCP。
...
* 相关接口：
  ...
  * 业务 REST 接口：
    ...
    * API：
      * `/api/*` 下各领域 REST 接口。
      * 当前普通用户侧受保护 endpoint 同时支持当前请求 cookie 中的站内 session 与 OAuth bearer token。
    * MCP：
      * 不适用。
  * 平台工具接口：
    * Web：
